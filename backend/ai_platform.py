...
""".strip()
    ...
    def _make_natural_language_summary(
        ...
        if self.llm_available:
            system_prompt = (
                "You are a senior B2B decision intelligence analyst. "
                "Generate a concise, human-first, cinematic executive narrative (1-2 sentences) "
                "summarizing the proposed path, identified risk/opportunity, and expected business impact. "
                "Do not include any greeting or conversational filler. Return only the summary text."
            )
            actions_str = ", ".join([a.title for a in next_best_actions])
            metrics_str = str(success_metrics)
            user_prompt = (
                f"Analysis:\n- Opportunities: {analysis.opportunities}\n- Risks: {analysis.risks}\n"
                f"- Gaps: {analysis.missing_information}\n"
                f"Proposed Actions: {actions_str}\n"
                f"Success Metrics: {metrics_str}\n\n"
                f"Generate a beautiful 1-2 sentence executive summary of the path forward."
            )
            try:
    ...
    def _make_natural_language_summary_legacy(
        self,
        analysis: OpportunityRisk,
        next_best_actions: List[NextBestAction],
        success_metrics: Dict[str, Any],
